...
public class GitlabAPI {
    ...
    private static String API_NAMESPACE = "/api/v3";
    ...
    private Proxy userProxy=null;

    private GitlabAPI(String hostUrl, String apiVersion, String apiToken, TokenType tokenType, AuthMethod method) {
        this.hostUrl = hostUrl.endsWith("/") ? hostUrl.replaceAll("/$", "") : hostUrl;
        this.apiToken = apiToken;
        this.tokenType = tokenType;
        this.authMethod = method;
        API_NAMESPACE = "/api/"+apiVersion;
    }
    
    
    private GitlabAPI(String hostUrl, String apiVersion, String apiToken,Proxy proxy, TokenType tokenType, AuthMethod method) {
        this.hostUrl = hostUrl.endsWith("/") ? hostUrl.replaceAll("/$", "") : hostUrl;
        this.apiToken = apiToken;
        this.tokenType = tokenType;
        this.authMethod = method;
    ...
    public static GitlabSession connect(String hostUrl, String apiVersion, String username, String password) throws IOException {
        String tailUrl = GitlabSession.URL;
        GitlabAPI api = connect(hostUrl, apiVersion,null, null, null);
        return api.dispatch().with("login", username).with("password", password)
                .to(tailUrl, GitlabSession.class);
    }

    public static GitlabAPI connect(String hostUrl,String apiVersion, String apiToken) {
        return new GitlabAPI(hostUrl,apiVersion, apiToken, TokenType.PRIVATE_TOKEN, AuthMethod.HEADER);
    }
    public static GitlabAPI connect(String hostUrl, String apiVersion,String apiToken, Proxy proxy) {
    	if(proxy == null){
    		return new GitlabAPI(hostUrl, apiVersion,apiToken, TokenType.PRIVATE_TOKEN, AuthMethod.HEADER);
    	}else{
            return new GitlabAPI(hostUrl, apiVersion, apiToken,proxy, TokenType.PRIVATE_TOKEN, AuthMethod.HEADER);
    	}
    }
    public static GitlabAPI connect(String hostUrl,String apiVersion, String apiToken, TokenType tokenType) {
        return new GitlabAPI(hostUrl,apiVersion, apiToken, tokenType, AuthMethod.HEADER);
    }

    public static GitlabAPI connect(String hostUrl,String apiVersion, String apiToken, TokenType tokenType, AuthMethod method) {
        return new GitlabAPI(hostUrl, apiVersion, apiToken, tokenType, method);
    }
    ...
    * @throws IOException
    */
   public List<GitlabProject> getProjectsByName(String name) throws IOException {
       String tailUrl = GitlabProject.URL;
       return retrieve().getAll(tailUrl+"?search="+name, GitlabProject[].class);
   }
    /**
     *
    ...
    /*  modified by yellowfin  MAR 2017 */
    
    public List<GitlabCommit> getCommitsOnSinglePage(Serializable projectId, Pagination pag,String branchOrTag) throws IOException
    {
    	final Query query = new Query();
    	if(branchOrTag!=null){
    		query.append("ref_name", branchOrTag);
    	...
    	if(pag !=null){
    		query.mergeWith(pag.asQuery());
    	}
    	
    	String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId)+"/repository" +GitlabCommit.URL +query;
    	
    	return this.retrieve().getOnePage(tailUrl, GitlabCommit[].class);
    }
    
    public List<GitlabMergeRequest> getMergeRequestOnSinglePage(Serializable projectId, Pagination pag) throws IOException
    {
    	final Query query = new Query();
    	if(pag !=null){
    		query.mergeWith(pag.asQuery());
    	...
    	return this.retrieve().getOnePage(tailUrl, GitlabMergeRequest[].class);
    }
    
    
    public List<GitlabIssue> getIssuesOnSinglePage(Serializable projectId, Pagination pag) throws IOException
    {
    	final Query query = new Query();
    	if(pag !=null){
    		query.mergeWith(pag.asQuery());
    	}
    	String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) +GitlabIssue.URL +query;
    	return this.retrieve().getOnePage(tailUrl, GitlabIssue[].class);
    }
    
    public List<GitlabPipeline> getPipelinesOnSinglePage(Serializable projectId, Pagination pag) throws IOException
    {
    	final Query query = new Query();
    	if(pag !=null){
    		query.mergeWith(pag.asQuery());
    	...
    	String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) +GitlabPipeline.URL +query;
    	return this.retrieve().getOnePage(tailUrl, GitlabPipeline[].class);
    }
    
    public List<GitlabBuild> getBuildsOnSinglePage(Serializable projectId, Pagination pag) throws IOException
    {
    	final Query query = new Query();
    	if(pag !=null){
    		query.mergeWith(pag.asQuery());
    	...
    	String tailUrl = GitlabProject.URL + "/" + sanitizeProjectId(projectId) +GitlabBuild.URL +query;
    	return this.retrieve().getOnePage(tailUrl, GitlabBuild[].class);
    }
    
    public List<GitlabCommit> getCommitsOnSinglePage(Serializable projectId, Pagination pag, String ref, String startDate, String endDate) throws IOException
    {
    	final Query query = new Query();
    	if(startDate!=null){
    		query.append("since", startDate);
    	}
    	
    	if(endDate!=null){
    		query.append("until", endDate);
    	}
    	if(ref!=null){
    		query.append("ref_name", ref);
    	}
    	if(pag !=null){
    		query.mergeWith(pag.asQuery());
    	}
